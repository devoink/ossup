...
export const PROJECT_CONFIG_FILENAME = ".ossup.json";
...
export function parseIndex(raw: unknown): OssupIndexConfig {
  ...
  const profiles: OssupIndexConfig["profiles"] = {};
...
export async function loadIndex(): Promise<OssupIndexConfig | null> {
  try {
...
export async function saveIndex(index: OssupIndexConfig): Promise<void> {
...
export function parseProjectBinding(raw: unknown): ProjectBinding {
  if (!raw || typeof raw !== "object") {
    throw new Error("Invalid .ossup.json: expected object");
  }
  const o = raw as Record<string, unknown>;
  const forbidden = ["accessKeyId", "accessKeySecret", "bucket", "region"];
  for (const key of forbidden) {
    if (key in o) {
      throw new Error(`.ossup.json must not contain "${key}"; use profiles only`);
    }
  }
...
export async function findProjectOssupJson(
...
export async function readProjectBinding(
  ...
  const path = await findProjectOssupJson(cwd);
  if (!path) return null;
...
export async function resolveActiveProfile(
  ...
  if (process.env.OSSUP_PROFILE?.trim()) {
    const name = process.env.OSSUP_PROFILE.trim();
    const valid = validateProfileName(name);
    if (valid !== true) throw new Error(valid);
  ...
  if (!index) {
    throw new Error(
      "ossup is not configured. Run: ossup setup  or  ossup profile add <name>",
    );
  }
...
export async function loadConfigWithProfile(
  ...
  if (!(await profileExists(resolved.name))) {
    throw new Error(
      `Profile "${resolved.name}" not found. Run: ossup profile list`,
    );
  }
...
export async function removeProfile(name: string): Promise<void> {
  ...
  if (index.defaultProfile === name) {
    throw new Error(
      `Cannot remove default profile "${name}". Run: ossup profile default <other>`,
    );
  }
