...
export async function runProfileList(): Promise<void> {
  ...
  if (rows.length === 0) {
    console.log("No profiles. Run: ossup setup  or  ossup profile add <name>");
    return;
  }
...
export async function runProfileAdd(options: ProfileAddOptions = {}): Promise<void> {
  ...
  if (!options.nonInteractive) {
    ...
    const bind =
      options.bindProject ??
      (await confirm({
        message: `在当前目录写入 .ossup.json，绑定 profile「${name}」？`,
        default: true,
      }));
    if (bind) {
...
export async function runProfileUse(name: string, cwd?: string): Promise<void> {
  ...
  if (valid !== true) throw new Error(valid);
  if (!(await profileExists(name))) {
    throw new Error(`Profile "${name}" not found. Run: ossup profile list`);
  }
...
export async function runProfileCommand(
  ...
  switch (sub) {
    ...
    case "use":
      if (!rest[0]) {
        console.error("Usage: ossup profile use <name>");
        return 1;
      }
    ...
    case "default":
      if (!rest[0]) {
        console.error("Usage: ossup profile default <name>");
        return 1;
      }
    ...
    case "remove":
      if (!rest[0]) {
        console.error("Usage: ossup profile rm <name>");
        return 1;
      }
    ...
    default:
      ...
      console.log(`Usage:
  ossup profile list
  ossup profile add [name]
  ossup profile show [name]
  ossup profile use <name>
  ossup profile default <name>
  ossup profile rm <name>`);
      return 1;
  }
}
